...
export const updateCustomerHandler = async (req, res) => {
  try {
    ...
    if (
      Array.isArray(customerData.userProfileDetails) &&
      customerData.userProfileDetails.length > 0
    ) {
...
export const applyForUPTHandler = async (req, res) => {
  try {
    ...
    const trainerRequest = await applyForUPT({
      customerId,
      trainerId,
      message,
    });
...
export const getTrainerPlanForCustomer = async (req, res) => {
  try {
    ...
    const trainer = assigned.trainer;

    res.status(200).json({
      success: true,
      data: {
        ...trainer.plan,   // now includes description
 ...
 */
export const updateUPTRequestStatusHandler = async (req, res) => {
  try {
    const { requestId } = req.params;
    const { status } = req.body;
    const result = await updateUPTRequestStatus({ requestId, status });
    res.status(200).json({
      success: true,
      message: `UPT request ${status.toLowerCase()} successfully.`,
      data: result,
    });
  } catch (err) {
